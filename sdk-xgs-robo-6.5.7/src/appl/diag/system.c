...
_display_current_shell_mode(int unit)
{
    ...
    case ESW_CMD_MODE:
        ...
        break;
    case ROBO_CMD_MODE:
        sal_strncpy(mode ,"ROBO", sizeof(mode));
        break;
#if defined(BCM_EA_SUPPORT)
    case EA_CMD_MODE:
        sal_strncpy(mode, "EA", sizeof(mode));
        break;
#endif
#if defined(BCM_SBX_SUPPORT)
    case SBX_CMD_MODE:
        sal_strncpy(mode ,"SBX", sizeof(mode));
        break;
#endif
    }
...
cmd_mode(int unit, args_t *a)
{
    ...
    if ((subcmd = ARG_GET(a)) == NULL) { /* Swap modes. */
        ...
        if (sal_strcasecmp(subcmd, "BCMX") == 0) {
            ...
            command_mode_set(unit, EA_CMD_MODE);
#endif
        } else {
            return CMD_USAGE;
        }
...
#endif /* BCM_ESW_SUPPORT || defined(BCM_SBX_SUPPORT) || defined(BCM_SAND_SUPPORT) */
#endif /* BCM_WARM_BOOT_SUPPORT */
...
diag_shell(void)
{
    ...
    for (i = 0; i < soc_ndev; i++) {
        ...
        if (soc_property_get_str(SOC_NDEV_IDX2DEV(i), "scache_nh") != NULL) {
            stable_location = _SHR_SWITCH_STABLE_DEVICE_NEXT_HOP;
            stable_flags = 0;
            stable_size = 10 * 1024;      /* 10 Kbytes */
        } else if (soc_property_get_str(SOC_NDEV_IDX2DEV(i), "scache_nh_basic") != NULL) {
            stable_location = _SHR_SWITCH_STABLE_DEVICE_NEXT_HOP;
            ...
            if ((SOC_CONTROL(SOC_NDEV_IDX2DEV(i)) != NULL) &&
               SOC_IS_TD_TT(SOC_NDEV_IDX2DEV(i))) {
                ...
                if(SOC_IS_APACHE(SOC_NDEV_IDX2DEV(i))) {
                    /* Default scache size 6MB is needed for apache */
        ...
        /* in WB regression always use shared memory */
        ...
        if (BCM_SWITCH_STABLE_NONE != stable_location) {
            ...
            if (!(stable_flags & SOC_STABLE_BASIC)) {
                if (soc_property_get(SOC_NDEV_IDX2DEV(i), "scache_diffserv", FALSE)) {
            ...
            if (flags & BOOT_F_WARM_BOOT && SOC_IS_ARAD(SOC_NDEV_IDX2DEV(i))) {
...
#endif /* BCM_ESW_SUPPORT || BCM_SBX_SUPPORT || BCM_SAND_SUPPORT) */
#endif /* BCM_WARM_BOOT_SUPPORT */

#if defined(BCM_SAND_SUPPORT)
    for (i = 0; i < soc_ndev; i++) {
#if defined(BCM_DNX_SUPPORT)
        if (SOC_IS_DNX(SOC_NDEV_IDX2DEV(i)))
        {
            if(cmd_dnx_verify(SOC_NDEV_IDX2DEV(i)) != CMD_OK)
            {
                LOG_VERBOSE(BSL_LS_APPL_SHELL, (BSL_META_U(SOC_NDEV_IDX2DEV(i), "Errors in Shell Commands Definition\n")));
                return;
            }
        }
#endif
#if defined(BCM_DNXF_SUPPORT)
        if (SOC_IS_DNXF(SOC_NDEV_IDX2DEV(i)))
        {
            if(cmd_dnxf_verify(SOC_NDEV_IDX2DEV(i)) != CMD_OK)
            {
                LOG_VERBOSE(BSL_LS_APPL_SHELL, (BSL_META_U(SOC_NDEV_IDX2DEV(i), "Errors in Shell Commands Definition\n")));
                return;
            }
    ...
    if(BCM_E_NONE == soc_ea_do_init(soc_ndev)){
        for(i = 0; i < soc_ndev; i++){
            if(soc_attached(SOC_NDEV_IDX2DEV(i)) && SOC_IS_TK371X(SOC_NDEV_IDX2DEV(i))){
                 int rv;
                 if ((rv = (bcm_init(SOC_NDEV_IDX2DEV(i)))) < 0) { _SHR_ERROR_TRACE(rv);  return; }
            }
        }
    }
#endif
