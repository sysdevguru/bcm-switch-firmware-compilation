...
#define LPM128_TCAM_NUM(u, i)       ((i) / SOC_L3_DEFIP_TCAM_DEPTH_GET(u))
...
#define LPM128_IN_ODD_TCAM_FIRST(u, i)  \
        (LPM128_IN_ODD_TCAM(u, i) && (((i) % SOC_L3_DEFIP_TCAM_DEPTH_GET(u)) == 0))
#define LPM128_IN_EVEN_TCAM_FIRST(u, i)  \
        (LPM128_IN_EVEN_TCAM(u, i) && (((i) % SOC_L3_DEFIP_TCAM_DEPTH_GET(u)) == 0))
...
void soc_fb_lpm_hash_verify(int u, void *entry_data, uint32 tab_index)
{
    ...
    if (tab_index >= SOC_LPM_STATE_START(u, MAX_PFX_INDEX)) {
        hash = SOC_LPM_STATE_HASH(u);
    }

    if (SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, MODE0f)) {
        /* IPV6 entry */
        valid = (SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, VALID1f) &&
                 SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, VALID0f));

        if ((SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, MODE0f) == 3) && 
            LPM128_IN_ODD_TCAM(u, tab_index)) {
            tab_index -= SOC_L3_DEFIP_TCAM_DEPTH_GET(u);
        ...
        /* IPV4 entry */
        valid = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, VALID0f);
...
extern int
dbg_soc_fb_lpm_reinit(int unit, int idx, defip_entry_t *lpm_entry);
extern int
dbg_soc_fb_lpm128_reinit_done(int unit, int ipv6);
extern int
dbg_soc_fb_lpm128_reinit(int u, int idx, defip_entry_t *lpm_entry,
...
debug_write_l3_defip(int u, int copyno, int index, void *entry_data)
{
    ...
    int i = 0, j = 0;
    int offset = (soc_mem_index_count(u, L3_DEFIPm) >> 1);
    ...
    rv = soc_mem_write(u, L3_DEFIPm, copyno, index, entry_data);
    if (rv < 0) {
        LOG_CLI(("L3 Defip write error \n"));
    }
    sal_memcpy(&(DBG_SOC_L3_DEFIP(u)[index]), entry_data, sizeof(defip_entry_t));
    rv = soc_mem_read(u, L3_DEFIPm, copyno, index, read);
    if (rv < 0) {
        LOG_CLI(("L3 Defip read error \n"));
    }
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, VALID0f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, MODE0f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR0f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR_MASK0f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, VRF_ID_0f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, VALID1f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, MODE1f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR1f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, IP_ADDR_MASK1f);
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, VRF_ID_1f);


    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, read, VALID0f);
    ...
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, read, VALID1f);
...
debug_read_l3_defip(int u, int copyno, int index, void *entry_data)
{
    ...
    rv = soc_mem_read(u, L3_DEFIPm, copyno, index, entry_data);
    if (rv < 0) {
        LOG_CLI(("L3 Defip read  error \n"));
    }
    data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, entry_data, VALID0f);
    ...
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), VALID0f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), MODE0f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), IP_ADDR0f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), IP_ADDR_MASK0f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), VRF_ID_0f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), VALID1f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), MODE1f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), IP_ADDR1f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), IP_ADDR_MASK1f);
    data1[j++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), VRF_ID_1f);

    if (sal_memcmp(data0, data1, sizeof(data0))) {
        soc_mem_entry_dump(u, L3_DEFIPm, entry_data);
        soc_mem_entry_dump(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]));
...
/*    int pfx_state_size2 = sizeof(soc_lpm_state_t) * (MAX_PFX_ENTRIES);*/
    ...
    SOC_LPM_STATE_F(DBG_SOC_LPM_STATE2(u), MAX_PFX_INDEX) = defip_size;
    SOC_LPM_STATE_S(DBG_SOC_LPM_STATE2(u), MAX_PFX_INDEX) = paired_size;
    SOC_LPM_STATE_E(DBG_SOC_LPM_STATE2(u), MAX_PFX_INDEX) = paired_size - 1;
    ...
    for (i = 0; i < paired_size; i++) {
        ...
        if (!soc_L3_DEFIPm_field32_get(u, &(DBG_SOC_L3_DEFIP(u)[i]), VALID0f) &&
            !soc_L3_DEFIPm_field32_get(u, &(DBG_SOC_L3_DEFIP(u)[i]), VALID1f)) {
            continue;
        }
...
soc_fb_lpm_debug_collect(uint32 u)
{
    ...
    if (DBG_SOC_L3_DEFIP(u)) {
        ...
        for (index = 0; index < soc_mem_index_count(u, L3_DEFIPm); index++) {
            ...
            data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), VALID0f);
            ...
            data0[i++] = soc_mem_field32_get(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[index]), VALID1f);
    ...
    sh_process_command(0, "d chg l3_defip");
    sh_process_command(0, "ser log");
    sh_process_command(0, "soc");
...
int _lpm_fb_urpf_entry_replicate(int u, int index, int temp_index, uint32 *e)
{
    ...
    int rv;               /* Return type.                                  */
    defip_entry_t lpm_entry; /* Temporary lpm entry. */ 
    ...
    if (!ipv6) { /* IPV4 */
        ...
        if (soc_feature(u, soc_feature_lpm_atomic_write)) {
           ...
           if (temp_index != -1) {
              ...
              sal_memcpy(e_temp, soc_mem_entry_null(u, L3_DEFIPm),
                         soc_mem_entry_words(u,L3_DEFIPm) * 4);
              /* Write temp buffer to End slot */ 
              rv = WRITE_L3_DEFIPm(u, MEM_BLOCK_ANY, temp_hw_index, e_temp);
              SOC_IF_ERROR_RETURN(rv);
...
STATIC int
_soc_fb_lpm_prefix_get_by_len(int u, int ipv6, int vrf_type, int len, int *pfx) 
{
    int         pfx_len = len;
    ...
    case SOC_L3_VRF_OVERRIDE:
        *pfx = pfx_len + 2 * (MAX_PFX_ENTRIES / 6);
        break;
    default:   
        *pfx = pfx_len +  (MAX_PFX_ENTRIES / 6);
        break;
    }
...
_soc_fb_lpm_prefix_length_get(int u, void *entry, int *pfx_len) 
{
    ...
    return _soc_fb_lpm_prefix_get_by_len(u, ipv6, vrf_id, pfx, pfx_len);
}
...
#define MAX_PFX_CONF_NUM    64
    ...
    for (i = 1; i <= MAX_PFX_CONF_NUM; i++) {
        ...
        (void)_soc_fb_lpm_prefix_get_by_len(u, ipver == 6, vrf_type, plen, &pfx);
    ...
    /* ipv6_64_depth equals to soc_mem_index_count(L3_DEFIPm) 
...
#ifdef FB_LPM_DEBUG
    SOC_LPM_STATE_F(DBG_SOC_LPM_STATE2(u), MAX_PFX_INDEX) = defip_table_size;
    SOC_LPM_STATE_S(DBG_SOC_LPM_STATE2(u), MAX_PFX_INDEX) = start_index;
    SOC_LPM_STATE_E(DBG_SOC_LPM_STATE2(u), MAX_PFX_INDEX) = start_index - 1;
#endif
#if defined(BCM_KATANA_SUPPORT)
    if (SOC_IS_KATANA(u)) {
...
soc_fb_lpm_reinit_done(int unit, int ipv6)
{
    ...
    for (idx = MAX_PFX_INDEX; idx >= 0; idx--) {
        if ((idx == MAX_PFX_INDEX) && (SOC_LPM_STATE_START(unit, idx) <= 0)) {
            continue;
        }
                                   ...
                                   int *v)
{
    if (index & FB_LPM_HASH_IPV6_MASK) {
        ...
        if (v) {
            *v = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, e, VALID0f) &&
                 SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, e, VALID1f) &&
                 SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, eupr, VALID0f) &&
                 SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, eupr, VALID1f);
        }
    } else {
        if (index & 0x1) {
            SOC_FB_LPM_HASH_ENTRY_IPV4_1_GET(u, e, r_entry);
                            ...
                            uint32 *rvt_index)
{
    ...
    if (old_index != FB_LPM_HASH_INDEX_NULL) {
        while(index != FB_LPM_HASH_INDEX_NULL && i++ < hash->entry_count) {
            ...
            SOC_IF_ERROR_RETURN(READ_L3_DEFIPm(u, MEM_BLOCK_ANY, rindex, e));
            if (index & FB_LPM_HASH_IPV6_MASK) {
                SOC_IF_ERROR_RETURN(READ_L3_DEFIPm(u, MEM_BLOCK_ANY, rindex + tcam_depth, eupr));
                               ...
                               uint32 *rvt_index0, uint32 *rvt_index1)
{
    ...
    if (mode) {
        ...
        if (SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, VALID1f) &&
            SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data, VALID0f) &&
            SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data_upr, VALID1f) &&
            SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry_data_upr, VALID0f)) {
...
soc_fb_lpm128_init(int u)
{
    ...
    DBG_SOC_L3_DEFIP(u) = sal_alloc(sizeof(defip_entry_t) * 
                                 soc_mem_index_count(u, L3_DEFIPm), "Debug L3_DEFIP");
...
_lpm128_fb_urpf_entry_replicate(int u, int index, int temp_index, uint32 *e, uint32 *eupr)
{
    ...
    uint32 mask1;         /* Mask 1 field value.                           */
    int def_gw_flag;      /* Entry is default gateway.                     */
    defip_entry_t defip_temp_e; /* Temporary lpm entry. */ 
...
lpm128_idx_to_pfx_dump(int u, int before)
{
    ...
    for (i = 0; i < defip_table_size; i++) {
        ...
        if (before == 2) {
            ...
            pfx2 = DBG_SOC_LPM128_INDEX_TO_PFX_GROUP2(u, i + tcam_depth);
        } else if (before == 1) {
            pfx1 = DBG_SOC_LPM128_INDEX_TO_PFX_GROUP(u, i);
            pfx2 = DBG_SOC_LPM128_INDEX_TO_PFX_GROUP(u, i + tcam_depth);
        } else {
            pfx1 = SOC_LPM128_INDEX_TO_PFX_GROUP(u, i);
            pfx2 = SOC_LPM128_INDEX_TO_PFX_GROUP(u, i + tcam_depth);
        }
...
dbg_soc_fb_lpm_reinit_done(int unit, int ipv6)
{
    ...
    SOC_LPM_STATE_P(state2, MAX_PFX_INDEX) = -1;

    for (idx = MAX_PFX_INDEX; idx >= 0; idx--) {
        if ((idx == MAX_PFX_INDEX) && (SOC_LPM_STATE_S(state2, idx) <= 0)) {
            continue;
        }
        ...
        if (SOC_LPM_PFX_IS_V4(unit, idx)) {
            ...
            if (!soc_feature(unit, soc_feature_half_of_l3_defip_ipv4_capacity)) {
                v0 = SOC_MEM_OPT_F32_GET(unit, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(unit)[from_ent]), VALID0f);
                v1 = SOC_MEM_OPT_F32_GET(unit, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(unit)[from_ent]), VALID1f);
...
dbg_soc_fb_lpm128_setup_pfx_state(int u, soc_lpm128_state_p lpm_state_ptr)
{
    ...
    for (idx = MAX_PFX128_INDEX; idx > 0; idx--) {
        ...
        if (SOC_LPM128_PFX_IS_V4(u, idx)) {
            ...
            if (!soc_feature(u, soc_feature_half_of_l3_defip_ipv4_capacity)) {
                v0 = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[from_ent]), VALID0f);
                v1 = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, &(DBG_SOC_L3_DEFIP(u)[from_ent]), VALID1f);
...
dbg_soc_fb_lpm128_reinit_done(int unit, int ipv6)
{
    ...
    for (i = start_idx; (next_pfx != -1) && i < end_idx; i++) {
        ...
        if (DBG_SOC_LPM128_INDEX_TO_PFX_GROUP2(unit, i) != -1) {
            ...
            if ((SOC_LPM128_PFX_IS_V6(u, pfx)) && ((i / tcam_depth) & 1)) {
                continue;
            }
                     ...
                     defip_entry_t *lpm_entry_upr)
{
    ...
    if (SOC_LPM128_STATE_VENT(u, lpm_state_ptr, pfx) == 0) {
        ...
        if (SOC_LPM128_STATE_END2(u, lpm_state_ptr, pfx) != -1) {
            ...
            if (((SOC_LPM128_PFX_IS_V6(u, pfx))) && ((idx / tcam_depth) & 1)) {
                return SOC_E_NONE;
            }
    ...
    if (SOC_LPM128_PFX_IS_V6(u, pfx)) {
        DBG_SOC_LPM128_INDEX_TO_PFX_GROUP2(u, idx + tcam_depth) = pfx;
    }
...
int lpm_state_verify(int u, int *err)
{
    ...
    free_count = SOC_LPM_STATE_F(lpm_state_ptr, MAX_PFX_INDEX);
...
int lpm128_state_verify(int u)
{
    ...
    while (curr_pfx != -1) {
        ...
        if (start2 != -1) {
            ...
            if (SOC_LPM128_PFX_IS_V4(u, curr_pfx)) {
                ...
                int v6_count_in_last_tcam = first_v4_start % tcam_depth;
                 ...
                 */
                end_idx = end1 + fent;
                if (end_idx / tcam_depth > end1 / tcam_depth  &&
                    start1 / tcam_depth == first_v4_start / tcam_depth &&
                    end1 / tcam_depth == first_v4_start / tcam_depth)  {
                ...
                if (next_pfx != -1) {
                    ...
                    if (end1 / tcam_depth == (start1_next_pfx - 1) / tcam_depth) {
                        end_idx = end1 + (fent / 2);
                    } else {
                        k = ((start1_next_pfx - 1) / tcam_depth - end1 / tcam_depth);
                        end_idx = end1 + (fent / 2) + (k / 2 * tcam_depth);
                    }

                    if(((start1_next_pfx - 1) / tcam_depth ) & 1) {
                        end_idx_comp = (start1_next_pfx - 1) - tcam_depth;
                    } else {
                    ...
                    k = (end1 + fent / 2) / tcam_depth - end1 / tcam_depth;
                    end_idx = end1 + fent / 2 + k * tcam_depth;

                    if (end_idx != (paired_size - tcam_depth - 1)) {
                        LOG_ERROR(BSL_LS_SOC_LPM,
                                  (BSL_META_U(u,
                                  "End_idx caculation wrong: end_idx %d ,"\
                                  "should be %d\n"), end_idx,
                                  (paired_size - tcam_depth - 1)
                                  ));
                        error_count++;
                    }
                }
        ...
        for (i = start1; i <= end_idx; i++) {
            if (SOC_LPM128_PFX_IS_V4(u, curr_pfx) &&
                ((i / tcam_depth) & 1) && ((i % tcam_depth) == 0) &&
                ((i / tcam_depth) - 1 == first_v4_start / tcam_depth)) {
            ...
            if (pfxmap[i] != -1) {
                ...
                pfxmap[i] = curr_pfx;
                if (SOC_LPM128_PFX_IS_V6(u, curr_pfx)) {
                    pfxmap[i + tcam_depth] = curr_pfx;
                }

                if (i <= end1) {
                    pfxmap2[i] = curr_pfx;
                    if (SOC_LPM128_PFX_IS_V6(u, curr_pfx)) {
                        pfxmap2[i + tcam_depth] = curr_pfx;
                    }
                }
            }
        }
    ...
    if (is_reserved) {
        ...
        for (; i < paired_size; i++) {
            if (((i / tcam_depth) & 1) && ((i % tcam_depth) == 0) &&
                (reserved_v6_last != -1) &&
                ((i / tcam_depth) - 1 == reserved_v6_last / tcam_depth)) {
...
_lpm128_v4_domain_odd_start(int u)
{
    ...
    if (pfx_id == -1) {
        ...
        if (fent >= 2  * rem) {
            ...
            v4_domain_odd_start = end + tcam_depth + (fent / 2) + 1;
        }
                       ...
                       int to_ent, int erase)
{
    ...
    if (fe) {
        sal_memcpy(e, fe, soc_mem_entry_words(u,L3_DEFIPm) * 4);
    } else {
        SOC_IF_ERROR_RETURN(READ_L3_DEFIPm(u, MEM_BLOCK_ANY, from_ent, e));
    }
    if (SOC_LPM128_PFX_IS_V6(u, pfx)) {
        ...
        if (feupr) {
            sal_memcpy(eupr, feupr, soc_mem_entry_words(u, L3_DEFIPm) * 4);
        } else {
            SOC_IF_ERROR_RETURN(READ_L3_DEFIPm(u, MEM_BLOCK_ANY,
        ...
        if (rv >= 0) {
            rv = WRITE_L3_DEFIPm(u, MEM_BLOCK_ANY, to_ent + tcam_depth, eupr);
        }
...
_lpm128_fb_shift_v4_pfx_up(int u, soc_lpm128_state_p lpm_state_ptr, int pfx)
{
    ...
    SOC_IF_ERROR_RETURN(soc_mem_cache_invalidate(u, L3_DEFIPm,
                                                 MEM_BLOCK_ANY, end));
#endif /* FB_LPM_TABLE_CACHED */
    SOC_IF_ERROR_RETURN(READ_L3_DEFIPm(u, MEM_BLOCK_ANY, end, e));
    v0 = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, e, VALID0f);
    v1 = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, e, VALID1f);
                                  ...
                                  soc_lpm128_state_p lpm_state_ptr)
{
    ...
    v4_odd_start = v6_domain_end + tcam_depth;
...
#endif /* FB_LPM_TABLE_CACHED */
    SOC_IF_ERROR_RETURN(READ_L3_DEFIPm(u, MEM_BLOCK_ANY, end_ent, e));
    v0 = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, e, VALID0f);
    v1 = SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, e, VALID1f);
                                         ...
                                         soc_lpm128_state_p lpm_state_ptr)
{
    ...
    if (_lpm128_pfx_conflicting(next_pfx, pfx)) {
        ...
        if (LPM128_TCAM_NUM(u, fent_start) ==
            ...
            fent2 = fent >> 1;
            other_start = fent_start + tcam_depth;
        } else {
                            ...
                            soc_lpm128_state_p lpm_state_ptr)
{
    ...
    while (free_pfx > pfx) {
        ...
        if (SOC_LPM128_STATE_FENT(u, lpm_state_ptr, free_pfx) &&
            ...
            other_start = v4_start + tcam_depth;
                         ...
                         void *e, int slot, int *is_deleted)
{
    ...
    if (to_ent != from_ent) {
        ...
        if (rv >= 0) {
            rv = WRITE_L3_DEFIPm(u, MEM_BLOCK_ANY, to_ent + tcam_depth, efupr);
        }
        if (rv >= 0) {
