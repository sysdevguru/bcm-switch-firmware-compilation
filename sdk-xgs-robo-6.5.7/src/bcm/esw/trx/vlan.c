...
#if (defined(BCM_TRIDENT2PLUS_SUPPORT) || defined(BCM_KATANA2_SUPPORT)) && defined(INCLUDE_L3)
...
#define PKT_VLAN_OUTER_ACTION_SET(unit, vent_ptr, _action) \
   do { \
...
#define PKT_VLAN_INNER_ACTION_SET(unit, vent_ptr, _action) \
   do { \
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) && defined(INCLUDE_L3)
#define SOC_MEM_EGR_VLAN_XLATE_FIELD_INFO_GET(m_unit, m_entry_data, m_field, m_field_data) \
        soc_meminfo_fieldinfo_field_get((uint32*)(m_entry_data), (&SOC_MEM_INFO(m_unit, EGR_VLAN_XLATEm)), \
            (m_field), (m_field_data))
#define SOC_MEM_EGR_VLAN_XLATE_FIELD_INFO_SET(m_unit, m_entry_data, m_field, m_field_data) \
        soc_meminfo_fieldinfo_field_set((uint32*)(m_entry_data), (&SOC_MEM_INFO(m_unit, EGR_VLAN_XLATEm)), \
            (m_field), (m_field_data))
#endif
        ...
        if (SOC_WARM_BOOT(unit)) {
            ...
            if (SOC_MEM_IS_VALID(unit, VLAN_MACm)) {
                for (i = 0; i < soc_mem_index_count(unit, VLAN_MACm); i++) {
                    ...
                    if (VLXLT_HASH_KEY_TYPE_VLAN_MAC != soc_mem_field32_get(
                        unit, VLAN_MACm, &vlan_mac_entry, KEY_TYPEf)) {
                        continue;
                    }
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        PBMP_ALL_ITER(unit, i) {
            ...
            if (soc_feature(unit, soc_feature_egr_vlan_control_is_memory)) {
                ...
                SOC_IF_ERROR_RETURN(
                        READ_EGR_VLAN_CONTROL_3m(unit, MEM_BLOCK_ANY, i, &entry));
        ...
        if (SOC_MEM_IS_VALID(unit,EGR_VLAN_XLATEm)) {
                                   ...
                                   bcm_vlan_action_set_t *action)
{
    ...
    if (soc_feature(unit, soc_feature_global_meter)) {
        if (SOC_IS_KATANA(unit) || SOC_IS_APACHE(unit)) {
 ...
 */
int
_bcm_trx_lltag_vlan_translate_entry_assemble(int unit, vlan_xlate_entry_t *vent,
                                      bcm_gport_t port,
                                      bcm_vlan_translate_key_t key_type,
    ...
    if (soc_feature(unit, soc_feature_lltag)) {
        ...
        switch (key_type) {
            case bcmVlanTranslateKeyPortPonTunnel:
                ...
                soc_VLAN_XLATEm_field32_set(unit, vent, KEY_TYPEf,
                                            key_type_value);
                soc_VLAN_XLATEm_field32_set(unit, vent, LLTAG__LLVIDf, lltag_vlan);
                break;
            case bcmVlanTranslateKeyPortPonTunnelOuter:
                ...
                soc_VLAN_XLATEm_field32_set(unit, vent, KEY_TYPEf,
                                            key_type_value);
                soc_VLAN_XLATEm_field32_set(unit, vent, LLTAG__LLVIDf, lltag_vlan);
                soc_VLAN_XLATEm_field32_set(unit, vent, LLTAG__OVIDf, other_vlan);
                break;
            case bcmVlanTranslateKeyPortPonTunnelInner:
                ...
                soc_VLAN_XLATEm_field32_set(unit, vent, KEY_TYPEf,
                                            key_type_value);
                soc_VLAN_XLATEm_field32_set(unit, vent, LLTAG__LLVIDf, lltag_vlan);
                soc_VLAN_XLATEm_field32_set(unit, vent, LLTAG__IVIDf, other_vlan);
                break;
            default:
                return BCM_E_PARAM;
        }
        ...
        if (use_glp) {
            ...
            if (BCM_GPORT_IS_TRUNK(port)) {
                ...
                soc_VLAN_XLATEm_field32_set(unit, vent, LLTAG__TGIDf, trunk_out);
            } else {
            ...
            soc_VLAN_XLATEm_field32_set(unit, vent, LLTAG__GLPf, 
                                      ...
                                      bcm_vlan_t outer_vlan)
{
    ...
    if (use_port_group) {
        ...
        soc_VLAN_XLATEm_field32_set(unit, vent, SOURCE_TYPEf, 3);
        BCM_IF_ERROR_RETURN(
            _bcm_trx_source_trunk_map_get(unit, port, PORT_GROUPf, &port_group));
        soc_VLAN_XLATEm_field32_set(unit, vent, SOURCE_FIELDf, port_group);
    } else
#endif /* BCM_TRIDENT2_SUPPORT */
    if (use_glp) {
                                                 ...
                                                 vlan_xlate_entry_t *return_ent)
{
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, VLAN_ACTION_VALIDf)) {
        value = soc_VLAN_XLATEm_field32_get(unit, vent, VLAN_ACTION_VALIDf); 
        soc_VLAN_XLATEm_field32_set(unit, return_ent, VLAN_ACTION_VALIDf,value);
    }

    if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, DISABLE_VLAN_CHECKSf)) {  
        value = soc_VLAN_XLATEm_field32_get(unit, vent, DISABLE_VLAN_CHECKSf);
        soc_VLAN_XLATEm_field32_set(unit, return_ent,
    ...
    if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, L3_IIF_VALIDf)) {
        value = soc_VLAN_XLATEm_field32_get(unit, vent, L3_IIF_VALIDf);
        soc_VLAN_XLATEm_field32_set(unit, return_ent, L3_IIF_VALIDf, value);
        value = soc_VLAN_XLATEm_field32_get(unit, vent, L3_IIFf);
        soc_VLAN_XLATEm_field32_set(unit, return_ent, L3_IIFf, value);
    }

#if defined(BCM_KATANA_SUPPORT) || defined(BCM_APACHE_SUPPORT)
    if (SOC_IS_KATANAX(unit) || SOC_IS_APACHE(unit)) {
...
#if defined(BCM_TSN_SR_SUPPORT)
    if (soc_feature(unit, soc_feature_tsn_sr)) {
        value = soc_VLAN_XLATEm_field32_get(unit, vent, SR_ENABLEf);
        soc_VLAN_XLATEm_field32_set(unit, return_ent, SR_ENABLEf, value);
        value = soc_VLAN_XLATEm_field32_get(unit, vent, SR_LAN_ID_VALIDf);
        soc_VLAN_XLATEm_field32_set(unit, return_ent, SR_LAN_ID_VALIDf, value);
        value = soc_VLAN_XLATEm_field32_get(unit, vent, SR_LAN_IDf);
        soc_VLAN_XLATEm_field32_set(unit, return_ent, SR_LAN_IDf, value);
    }
#endif /* BCM_TSN_SR_SUPPORT*/
   return BCM_E_NONE;
}
...
_bcm_trx_vlan_translate_action_entry_set(int unit, vlan_xlate_entry_t *vent)
{
    ...
    if (rv == SOC_E_NONE) {
         ...
         old_profile_idx = soc_VLAN_XLATEm_field32_get(unit, &return_vent,
                                                  TAG_ACTION_PROFILE_PTRf);
         BCM_IF_ERROR_RETURN(
             _bcm_trx_vlan_translate_action_entry_update (unit, 
                     vent, &return_vent));
         ...
         if ((rv == SOC_E_NONE) &&
             (old_profile_idx != ING_ACTION_PROFILE_DEFAULT)) {
             BCM_IF_ERROR_RETURN(
                                  ...
                                  bcm_vlan_action_set_t *action)
{
    ...
    if (soc_feature(unit, soc_feature_niv)) {
        ...
        if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, VIF__L3_IIF_VALIDf)) {
            ...
            if ((action->ingress_if >= min_l3_iif) &&
                (action->ingress_if <= max_l3_iif)) {
                soc_VLAN_XLATEm_field32_set(unit, &vent, VIF__L3_IIF_VALIDf, 1);
                soc_VLAN_XLATEm_field32_set(unit, &vent, VIF__L3_IIFf,
                                            action->ingress_if);
            }
        }
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, CLASS_IDf) &&
                (action->class_id)) {
            soc_VLAN_XLATEm_field32_set(unit, &vent, CLASS_IDf, action->class_id);
        } else
#endif
                                  ...
                                  bcm_vlan_action_set_t *action)
{
    ...
    if (key_type == bcmVlanTranslateKeyPortPonTunnel ||
        ...
        if (SOC_MEM_FIELD_VALID(unit, VLAN_XLATEm, MPLS_ACTIONf)) {
            soc_VLAN_XLATEm_field32_set(unit, &vent, MPLS_ACTIONf, 1);
        }
    }
    BCM_IF_ERROR_RETURN
        (_bcm_trx_vlan_action_profile_entry_add(unit, action, &profile_idx));
    ...
    if (soc_feature(unit, soc_feature_tsn_sr)) {
        if (action->flags & BCM_VLAN_ACTION_SET_SR_ENABLE) {
            soc_VLAN_XLATEm_field32_set(unit, &vent, SR_ENABLEf, 1);
            soc_VLAN_XLATEm_field32_set(unit, &vent, SR_LAN_ID_VALIDf, 1);
            if (action->flags & BCM_VLAN_ACTION_SET_SR_LAN_ID) {
                soc_VLAN_XLATEm_field32_set(unit, &vent, SR_LAN_IDf, 1);
            }
        }
    }
#endif /* BCM_TSN_SR_SUPPORT*/
    ...
    if (BCM_GPORT_IS_NIV_PORT(port)) {
        ...
        if ((rv == SOC_E_NONE) && 
            ...
            profile_idx = soc_VLAN_XLATEm_field32_get(unit, &vent,
                                        VIF__TAG_ACTION_PROFILE_PTRf);       
            /* Delete the old vlan action profile entry */
            if (profile_idx != ING_ACTION_PROFILE_DEFAULT) {
                rv = _bcm_trx_vlan_action_profile_entry_delete(unit,
                                                               profile_idx);
            }
        }
...
#endif /* BCM_KATANA_SUPPORT or  BCM_APACHE_SUPPORT */
...
_bcm_trx_vlan_translate_action_delete_all(int unit) 
{
    ...
    for(i = 0; i < nent; i++) {
        ...
        if ((rv >= 0) && (old_profile_idx != ING_ACTION_PROFILE_DEFAULT)) {
            /* Delete the old vlan action profile entry */
            rv = _bcm_trx_vlan_action_profile_entry_delete(unit, old_profile_idx);
        }
                                        ...
                                        bcm_vlan_action_set_t *action)
{
    ...
    bcm_gport_t trunk_gport = -1;
    int port_is_trunk_first_member = 0;
    bcm_trunk_t temp_trunk_id = -1;
    int temp_stm_idx = 0;
    source_trunk_map_table_entry_t temp_stm_entry;
#if defined(BCM_KATANA2_SUPPORT) && defined(INCLUDE_L3)
    ...
    if (BCM_GPORT_IS_TRUNK(port)) {
        ...
        BCM_IF_ERROR_RETURN(
                  _bcm_gport_modport_hw2api_map(unit, mod_out, port_out,
                                                      &mod_api, &port_api));
        /* Add a new port to the trunk, configure the port as the old members */
        rv = bcm_esw_trunk_find(unit, mod_api, port_api, &temp_trunk_id);
        if (rv == BCM_E_NONE) {
            BCM_IF_ERROR_RETURN
                (bcm_esw_trunk_get(unit, temp_trunk_id, NULL, 0, NULL,
                                   &port_count));
            if (port_count > 1) {
                member_array = sal_alloc(sizeof(bcm_trunk_member_t) * port_count,
                        "trunk member array");
                ...
                rv = bcm_esw_trunk_get(unit, temp_trunk_id, NULL, port_count,
                        member_array, &port_count);
                if (BCM_FAILURE(rv)) {
                    goto cleanup;
                }
                ...
                /* if the first trunk member is the new add port, use the second one*/
                if ((mod_array[0] == mod_out) && (port_array[0] == port_out)) {
                    rv = _bcm_esw_src_mod_port_table_index_get(unit, mod_array[1],
                            port_array[1], &stm_idx);
                } else {
                ...
                rv = _bcm_esw_src_mod_port_table_index_get(unit, mod_out,
                                           port_out, &temp_stm_idx);
                if (rv < 0) {
                    goto cleanup;
                }
                rv = READ_SOURCE_TRUNK_MAP_TABLEm(unit, MEM_BLOCK_ANY,
                                              temp_stm_idx, &temp_stm_entry);
                if (rv < 0) {
                    goto cleanup;
                }
                ...
                if (inner_vlan_low != BCM_VLAN_INVALID) {
                    ...
                    old_inner_idx = soc_mem_field32_get(unit, SOURCE_TRUNK_MAP_TABLEm,
                                                        &temp_stm_entry, field);
                    new_inner_idx = soc_mem_field32_get(unit, SOURCE_TRUNK_MAP_TABLEm,
                                                        &stm_entry, field);
                    ...
                    rv = soc_mem_field32_modify(unit,
                            SOURCE_TRUNK_MAP_TABLEm, temp_stm_idx, field, new_inner_idx);
                    if (rv < 0) {
                        goto cleanup;
                    }
                ...
                if (outer_vlan_low != BCM_VLAN_INVALID) {
                    ...
                    old_idx = soc_mem_field32_get(unit, SOURCE_TRUNK_MAP_TABLEm,
                                                        &temp_stm_entry, field);
                    new_idx = soc_mem_field32_get(unit, SOURCE_TRUNK_MAP_TABLEm,
                                              &stm_entry, field);
                    /* Get the profile table entry for this port/trunk */
                    _bcm_trx_vlan_range_profile_entry_get(unit,
                                            min_inner_vlan, max_inner_vlan, new_idx);
                    rv = _bcm_trx_vlan_range_profile_entry_add(unit, 
                            min_inner_vlan, max_inner_vlan, &new_idx);
                    if (rv < 0) {
                        goto cleanup;
                    ...
                    rv = soc_mem_field32_modify(unit,
                            SOURCE_TRUNK_MAP_TABLEm, temp_stm_idx, field, new_idx);
                    if (rv < 0) {
                        goto cleanup;
                    }
                ...
                goto cleanup;
            } else if (1 == port_count) {
                BCM_GPORT_TRUNK_SET(trunk_gport, temp_trunk_id);
                port_is_trunk_first_member = 1;
            }
        } else if (rv != BCM_E_NOT_FOUND){
             return rv;
        }
                                           ...
                                           bcm_vlan_t inner_vlan_high)
{
    ...
    uint32 new_inner_idx = 0;
    bcm_gport_t trunk_gport = -1;
    int port_is_trunk_last_member = 0;
    bcm_trunk_t temp_trunk_id = -1;
#if defined(BCM_KATANA2_SUPPORT) && defined(INCLUDE_L3)
    int pon_tunnel = FALSE;
    int nh_index, vp;
        ...
        if (rv == BCM_E_NONE) {
            BCM_IF_ERROR_RETURN
                (bcm_esw_trunk_get(unit, temp_trunk_id, NULL, 0, NULL,
                                   &port_count));
            if (port_count > 1) {
                /* Delete the configuconfigu for the trunk member*/
                BCM_IF_ERROR_RETURN(_bcm_esw_src_mod_port_table_index_get(unit,
                                                 mod_out, port_out, &stm_idx));
                BCM_IF_ERROR_RETURN(READ_SOURCE_TRUNK_MAP_TABLEm(unit, 
                ...
                if (inner_vlan_low != BCM_VLAN_INVALID) {
                    ...
                    BCM_IF_ERROR_RETURN(_bcm_trx_vlan_range_profile_entry_add(unit, 
                            min_inner_vlan, max_inner_vlan, &new_inner_idx));
                    BCM_IF_ERROR_RETURN(soc_mem_field32_modify(unit, 
                        SOURCE_TRUNK_MAP_TABLEm, stm_idx, field, new_inner_idx));
                    BCM_IF_ERROR_RETURN(_bcm_trx_vlan_range_profile_entry_delete(unit,
                                                                     old_inner_idx));
                }
                ...
                if (outer_vlan_low != BCM_VLAN_INVALID) {
                    ...
                    /* Get the profile table entry for this port/trunk */
                    _bcm_trx_vlan_range_profile_entry_get(unit,
                                            min_inner_vlan, max_inner_vlan, old_idx);
                    /* Find the min/max range. */
                    for (i = 0 ; i < 8 ; i++) {
                        if ((min_inner_vlan[i] == inner_vlan_low) &&
                            (max_inner_vlan[i] == inner_vlan_high)) {
                                              ...
                                              bcm_vlan_t inner_vlan)
{
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) && defined(INCLUDE_L3)
    if (inner_vlan == BCM_VLAN_INVALID) {
        soc_EGR_VLAN_XLATEm_field32_set(unit, vent, 
    ...
    if (BCM_GPORT_IS_TUNNEL(port_class)) {
...
#endif /* BCM_TRIDENT2PLUS_SUPPORT && INCLUDE_L3 */ 
        {
            ...
            if(SOC_IS_TRIUMPH3(unit)) {
                soc_EGR_VLAN_XLATEm_field32_set(unit, vent, KEY_TYPEf, 1); /* DVP */
            } else
#endif /* BCM_TRIUMPH3_SUPPORT */
            {
...
#endif
    {
        ...
        if (inner_vlan == BCM_VLAN_INVALID) {             
            soc_EGR_VLAN_XLATEm_field32_set(unit, vent, ENTRY_TYPEf, 
                        TD2PLUS_EVLXLT_HASH_KEY_TYPE_VFI_DVP_GROUP); /* VFI + DVP Group*/
            soc_EGR_VLAN_XLATEm_field32_set(unit, vent, 
...
#endif /* BCM_TRIDENT2PLUS_SUPPORT && INCLUDE_L3 */ 
        {
            ...
            if (BCM_GPORT_IS_MODPORT(port_class) &&
                ...
                if (modid != -1 && port_class != -1) {
                    soc_EGR_VLAN_XLATEm_field32_set(unit, vent, DST_MODIDf,
                            modid);
                    soc_EGR_VLAN_XLATEm_field32_set(unit, vent, DST_PORTf,
                            port_class);
                    soc_EGR_VLAN_XLATEm_field32_set(unit, vent,
...
#endif
            {
                soc_EGR_VLAN_XLATEm_field32_set(unit, vent, PORT_GROUP_IDf,
                        port_class);
            }
        }
    }
                                         ...
                                         bcm_vlan_action_set_t *action)
{
    ...
    egr_vlan_xlate_entry_t *vent_tmp_p = NULL, vent_tmp;
#endif /* BCM_TRIDENT2PLUS_SUPPORT && INCLUDE_L3 */ 
    ...
    if (inner_vlan == BCM_VLAN_INVALID) {     
        ...
        if (BCM_GPORT_IS_SET(port_class) &&
            ...
            field_info = soc_mem_fieldinfo_get(unit, EGR_VLAN_XLATEm, 
        ...
        SOC_MEM_EGR_VLAN_XLATE_FIELD_INFO_GET(unit, vent_ptr, 
                                         field_info, entry);
        /* Reuse the format of field XLATE_DATA in XLATE view
            * for VLAN_XLATE_VFI and VFI_DVP view. 
            */                                 
        vent_tmp_p = vent_ptr;
        field_info_1 = soc_mem_fieldinfo_get(unit, EGR_VLAN_XLATEm, 
                                             XLATE_DATAf);         
        if (!field_info_1) {
            return BCM_E_UNAVAIL;        
        }         
        sal_memset(&vent_tmp, 0, sizeof(egr_vlan_xlate_entry_t));        
        vent_ptr = &vent_tmp;
        SOC_MEM_EGR_VLAN_XLATE_FIELD_INFO_SET(unit, vent_ptr,
                                         field_info_1, entry); 
    }
#endif /* BCM_TRIDENT2PLUS_SUPPORT && INCLUDE_L3 */
    ...
    if (soc_feature(unit, soc_feature_vlan_pri_cfi_action)) {
        ...
        if (action->priority == -1) {
            ...
            PKT_VLAN_OUTER_ACTION_SET(unit, vent_ptr, action);
            PKT_VLAN_INNER_ACTION_SET(unit, vent_ptr, action);
...
#undef PKT_PRI_DFLT_ACTION
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
    if (SOC_MEM_FIELD_VALID(unit, EGR_VLAN_XLATEm, CLASS_IDf) &&
            (action->class_id)) {
        soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, CLASS_IDf, action->class_id);
        soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, CLASS_ID_VALIDf,1);
    }
#endif
    ...
    if (soc_feature(unit, soc_feature_tsn_sr)) {
        if (action->flags & BCM_VLAN_ACTION_SET_SR_ENABLE) {
            soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, SR_ENABLEf, 1);
            soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, SR_LAN_ID_VALIDf, 1);
            if (action->flags & BCM_VLAN_ACTION_SET_SR_LAN_ID) {
                soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, SR_LAN_IDf, 1);
            }
        } else {
            soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, SR_ENABLEf, 0);
            soc_EGR_VLAN_XLATEm_field32_set(unit, vent_ptr, SR_LAN_ID_VALIDf, 0);
        }
    }
#endif /* BCM_TSN_SR_SUPPORT*/
    if (rv == SOC_E_NONE) { 
                                         ...
                                         bcm_vlan_action_set_t *action)
{
    ...
    if (inner_vlan == BCM_VLAN_INVALID) { 
        ...
        SOC_MEM_EGR_VLAN_XLATE_FIELD_INFO_GET(unit, &res_vent, 
                                         field_info, entry);
        field_info_1 = soc_mem_fieldinfo_get(unit, EGR_VLAN_XLATEm, 
                                             XLATE_DATAf);         
        if (!field_info_1) {
        ...
        SOC_MEM_EGR_VLAN_XLATE_FIELD_INFO_SET(unit, &res_vent, 
                                         field_info_1, entry); 
    }
#endif /* BCM_TRIDENT2PLUS_SUPPORT && INCLUDE_L3 */
                                     ...
                                     bcm_vlan_t *inner_vlan)
{
    ...
    switch (key_type) {
      ...
      case bcmVlanTranslateKeyPortCapwapPayloadDouble:
#if defined(BCM_TRIDENT2PLUS_SUPPORT) && defined(INCLUDE_L3)
          if ((mem == EGR_VLAN_XLATEm) && soc_feature(unit, soc_feature_vp_sharing) &&
             (soc_mem_field32_get_def(unit, mem, vent, ENTRY_TYPEf, FALSE) == 0xc)) {
    ...
    if (mem == EGR_VLAN_XLATEm) {
...
#endif
        {
            ...
            if (soc_feature(unit, soc_feature_egr_vxlate_supports_dgpp)) {
                if(soc_EGR_VLAN_XLATEm_field32_get(unit, vent,
                                                   PORT_TYPE_DGPPf)) {
                    bcm_module_t modid =
 ...
 */
int
_bcm_trx_vlan_port_protocol_delete_all(int unit, int port)
{
                                  ...
                                  int vp)
{
    ...
    if (soc_feature(unit, soc_feature_global_meter) &&
       (SOC_IS_KATANAX(unit) || SOC_IS_APACHE(unit))) {
        rv = soc_mem_search(unit, VLAN_XLATEm, MEM_BLOCK_ANY, &idx, &vent, &current_vent, 0);
        if (rv == BCM_E_NONE) {
            rv = _bcm_esw_get_policer_from_table(unit, VLAN_XLATEm, 0, &current_vent,
                                                        &current_policer, 1);
            if (current_policer) {
