/*
 * $Id: link.c,v 1.211 Broadcom SDK $
...
#define LC_LOCK(unit) \
        if(SOC_IS_SABER2(unit) || SOC_IS_TD2_TT2(unit)) { \
...
#define BCM_LINK_PBMP_SCACHE_NUM        BCM_LINK_PBMP_SCACHE_NUM_WB_1_2
                                 ...
                                 int speed, int duplex)
{
    ...
    PBMP_ITER(pbmp, dst_port) {
        BCM_IF_ERROR_RETURN(bcm_esw_port_speed_get(unit, dst_port,
                                                   &dst_port_speed));
        if (dst_port_speed <= speed) {
            if (dst_port < 32) {
                new_val0 |= 1 << (dst_port % 32);
            } else if ((dst_port >=32) && (dst_port < 64)) {
                new_val1 |= 1 << (dst_port % 32);
            } else { /* dst_port >= 64 */
                new_val2 |= 1 << (dst_port % 32);
            }
        }
    }
    ...
    if (change) {                                                   /* 2) */
        ...
        if (lc->lc_f[port]) {
            ...
            if((rv == BCM_E_NONE) && (cur_link != new_link)){
                rv = _bcm_esw_linkscan_serdes_link_update(unit,port,new_link);
            }
        }
#ifdef BCM_TRIUMPH3_SUPPORT
                    ...
                    if(eee_en && new_link) {
                        ...
                        if (SOC_USE_PORTCTRL(unit)) {
                            if (current_time > lc->lc_up_time[port-1]) {
                                bcmi_esw_portctrl_eee_enable_set(unit, port, 1);
                            }
                        }
                        else {
                            _bcm_port_info_access(unit, port, &port_info);
            ...
            if(soc_feature(unit, soc_feature_eee)) {
                ...
                if (MAC_CONTROL_GET(port_info->p_mac, unit, port,
                    ...
                    if (new_link &&
                        ...
                        lc->lc_up_time[port-1] = (remote_advert.eee) ?
                            sal_time() : -1;
                    } else {
...
{
    soc_reg_t reg;
    soc_field_t soft_reset_field = SOFT_RESETf;
    int rv = BCM_E_NONE;
    uint64 ctrl;
    int soft_reset_val = 0;

    if (!(SOC_IS_TRIDENT2PLUS(unit) || SOC_IS_TOMAHAWKX(unit) ||
        SOC_IS_TRIDENT(unit) || SOC_IS_TRIDENT2(unit))) {
        return TRUE;
    }
    if (IS_HG_PORT(unit, port) || IS_XE_PORT(unit, port) ||
        IS_CE_PORT(unit, port)) {
        ...
        rv = soc_reg_get(unit, reg, port, 0, &ctrl);
        if (SOC_E_NONE != rv) {
            return TRUE;
        }
        soft_reset_val = soc_reg64_field32_get(unit, reg, ctrl, soft_reset_field);
        if (soft_reset_val == 1) {
            return TRUE;
        } else {
            return FALSE;
...
_bcm_esw_linkscan_thread(int unit)
{
    ...
    if (!lc->lc_warm_boot) {
        ...
        PBMP_ITER(PBMP_PORT_ALL(unit), port) {
            /* In Cold boot, irrespective of link scan mode, link state
             * is retrieved from PHY link status.
             */
    ...
    while ((interval = lc->lc_us) != 0) {
        ...
        if (normal_scheduled || hw_force_scan) {
            _bcm_esw_linkscan_update(unit, 0 /*flags*/, PBMP_PORT_ALL(unit));
...
bcm_esw_linkscan_init(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_preemption)) {
        bcm_esw_linkscan_register(unit, bcmi_esw_preemption_linkscan_cb);
    }
#endif /* BCM_PREEMPTION_SUPPORT */
    return (BCM_E_NONE);
}
...
#ifdef BCM_PREEMPTION_SUPPORT
    if (soc_feature(unit, soc_feature_preemption)) {
        bcm_esw_linkscan_unregister(unit, bcmi_esw_preemption_linkscan_cb);
    }
#endif /* BCM_PREEMPTION_SUPPORT */
...
bcm_esw_linkscan_port_attach(int unit, bcm_port_t port)
{
    ...
    lc->lc_error[port].limit =
        soc_property_port_get(unit, port, spn_BCM_LINKSCAN_MAXERR, 5);
    lc->lc_error[port].delay =
        soc_property_port_get(unit, port, spn_BCM_LINKSCAN_ERRDELAY, 10000000);
    LC_UNLOCK(unit);
