                        ...
                        int cf_max, int *cf_count)
{
    ...
    *cf_count = 0;
     ...
     */
    for (bidx = 9; bidx >= (9 - num_banks + 1); bidx--) {                     
                                ...
                                int idx, _bcm_l3_cfg_t *l3cfg, int *nh_idx)
{
    ...
    switch (mem) {
    case L3_ENTRY_IPV4_UNICASTm:
        if (key_type != TD2_L3_HASH_KEY_TYPE_V4UC) {
    ...
    case L3_ENTRY_IPV4_MULTICASTm:
        if (key_type == TD2_L3_HASH_KEY_TYPE_V4MC) {
            l3cfg->l3c_flags = BCM_L3_IPMC;
        } else if (key_type == TD2_L3_HASH_KEY_TYPE_V4UC_EXT) {
            l3cfg->l3c_flags = 0;
                             ...
                             _bcm_l3_cfg_t *l3cfg)
{
    ...
    if (ipv6) {
        ...
        if ((l3cfg->l3c_flags & BCM_L3_L2ONLY) ||
            ...
            soc_mem_field32_set(unit, mem, buf_p, IPV6MC__L3_IIFf, l3cfg->l3c_ing_intf);
            key_type = TD2_L3_HASH_KEY_TYPE_V6MC;
        }
        for (idx = 0; idx < 4; idx++) {
            /* Set key type */
                          ...
                          l3_entry_ipv6_multicast_entry_t *l3x_entry)
{
    ...
    if (soc_feature(unit, soc_feature_multi_pipe_mapped_ports)) {
        ...
        if (soc_mem_field32_get(unit, mem, buf_p, svp_valid)) {
            l3cfg->l3c_port_tgid = soc_mem_field32_get(unit, mem, buf_p, source_vp);
            ...
            if (soc_mem_field32_get(unit, mem, buf_p, trunk_port)) {
                ...
                if ((((l3cfg->l3c_port_tgid & TH_IPMC_NO_SRC_CHECK_PORT)
                ...
                l3cfg->l3c_modid = soc_mem_field32_get(unit, mem, buf_p, module_id);
                l3cfg->l3c_port_tgid = soc_mem_field32_get(unit, mem, buf_p, port_num);
                l3cfg->l3c_tunnel = 0;
            }
        }
    } else if (soc_feature(unit, soc_feature_two_ingress_pipes)) {
...
bcm_td2_l3_replace(int unit, _bcm_l3_cfg_t *l3cfg)
{
    ...
    if (BCM_XGS3_L3_MCAST_ENTRY(l3cfg)) {
        ...
        if (SOC_IS_TOMAHAWKX(unit) &&
            ...
            mpath && (entry.l3c_flags & BCM_L3_MULTIPATH)) {
                BCM_IF_ERROR_RETURN(bcm_th_l3_egress_ecmp_rh_shared_copy(unit,
                                    nh_idx_old, nh_idx_new));
        }
#endif /* BCM_TOMAHAWK_SUPPORT */
    ...
    for (tbl = 0; tbl < num_tables; tbl++) {
        ...
        for (idx = 0; idx <= idx_max; idx++) {
            ...
            if (l3cfg.l3c_flags & BCM_L3_MULTIPATH) {
                ...
                if (BCM_XGS3_L3_EGRESS_MODE_ISSET(unit)) {
                    rv = bcm_xgs3_ecmp_group_del(unit, nh_idx);
                }
            } else {
 ...
 _bcm_td2_l3_ipmc_add(int unit, _bcm_l3_cfg_t *l3cfg)
{
   ...
   rv = _bcm_td2_l3_ipmc_ent_init(unit, (uint32 *)&l3x_entry, l3cfg);
   if (BCM_FAILURE(rv)) {
       return rv;
   }
            ...
            _bcm_defip_cfg_t           *defip_copy)
{
    ...
    if (info->l3a_flags & BCM_L3_IP6) {
        ...
        if (defip.defip_sub_len > 64) {
            if (SOC_PERSIST(unit)->memState[L3_DEFIP_PAIR_128m].index_max >= 0) {
    ...
    if (soc_feature(unit, soc_feature_alpm) &&
        ...
        if (defip.defip_alpm_cookie & SOC_ALPM_LOOKUP_HIT) {
            ...
            if (mem == L3_DEFIP_ALPM_IPV6_64_1m ||
                mem == L3_DEFIP_ALPM_IPV4_1m) {
                            ...
                            bcm_stat_value_t    *counter_values)
{
    ...
    BCM_IF_ERROR_RETURN(_bcm_td2_l3_route_stat_get_table_info(
                        unit, info, &num_of_tables, &table_info[0], NULL));
...
_bcm_td2_nh_nat_id_update(int unit, int key_type, void *val, int nat_id)
{
    ...
    if ((BCM_XGS3_L3_INGRESS_INTF_MAP_MODE_ISSET(unit)) &&
        ...
        rv = bcm_xgs3_l3_tbl_dma(unit, VLAN_MPLSm,
                                 sizeof(vlan_mpls_entry_t), "vlan_mpls_tbl",
    ...
    for (idx = 0; idx < BCM_XGS3_L3_NH_TBL_SIZE(unit); idx++) {
        ...
        if (key_type == _BCM_NH_UPDATE_KEY_L3_INTF) {
            ...
            if (key_type == _BCM_NH_UPDATE_KEY_VLAN) {
                ...
                if (BCM_XGS3_L3_INGRESS_INTF_MAP_MODE_ISSET(unit)) {
                    vlan_mpls_entry_ptr =
                        soc_mem_table_idx_to_pointer(unit, VLAN_MPLSm,
                                                     uint32 *,
                                                     vlan_mpls_tbl_ptr,
                                                     vlan);
                    /* coverity[forward_null : FALSE] */
                    l3_iif = soc_mem_field32_get(unit, VLAN_MPLSm,
                                                 vlan_mpls_entry_ptr,
                                                 L3_IIFf);
                } else {
                    l3_iif = vlan;
                                              ...
                                              _bcm_l3_intf_cfg_t *intf_info)
{
    ...
    if (BCM_XGS3_L3_INGRESS_INTF_MAP_MODE_ISSET(unit)) {
        if (SOC_MEM_FIELD_VALID(unit, VLAN_MPLSm, L3_IIFf)) {
            ...
            BCM_IF_ERROR_RETURN(
                soc_mem_read(unit, VLAN_MPLSm, MEM_BLOCK_ANY, vid, &vlan_mpls));
            iif.intf_id = soc_mem_field32_get(unit, VLAN_MPLSm,
                                              &vlan_mpls, L3_IIFf);
        }
    }
                           ...
                           defip_entry_t *lpm_entry, int nh_ecmp_idx, uint32 *flags)
{
    ...
    if (soc_feature(unit, soc_feature_ipmc_defip) &&  
        ...
        soc_L3_DEFIPm_field32_set(unit, lpm_entry, L3MC_INDEX0f, lpm_cfg->defip_mc_group);
 ...
 */
STATIC INLINE void
_bcm_td2_alpm_lpm_data_parse(int unit, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx,
                             defip_entry_t *lpm_entry)
{
    ...
    if (soc_feature(unit, soc_feature_ipmc_defip) &&
            soc_L3_DEFIPm_field32_get(unit, lpm_entry, MULTICAST_ROUTE0f)) {
        int val;
        ...
        lpm_cfg->defip_mc_group = soc_L3_DEFIPm_field32_get(unit, lpm_entry, L3MC_INDEX0f);
    }
#endif
                                ...
                                int *nh_ecmp_idx, defip_pair_128_entry_t *lpm_entry)
{
    ...
    /* Check if entry points to ecmp group. */
    if (soc_L3_DEFIP_PAIR_128m_field32_get(unit, lpm_entry, ECMPf)) {
        /* Mark entry as ecmp */
        lpm_cfg->defip_ecmp = 1;
        lpm_cfg->defip_flags |= BCM_L3_MULTIPATH;
...
#if defined(BCM_TRIDENT2PLUS_SUPPORT) || defined(BCM_APACHE_SUPPORT)
    if (soc_feature(unit, soc_feature_ipmc_defip) &&
            (soc_L3_DEFIP_PAIR_128m_field32_get(unit, lpm_entry, MULTICAST_ROUTEf))) {
...
_bcm_td2_alpm_128_clear_hit(int u, void *key_data, int index, int flex)
{
    ...
    BCM_IF_ERROR_RETURN(
        soc_mem_read_no_cache(u, 0, mem, 0, MEM_BLOCK_ANY, index, e));
    if (SOC_MEM_FIELD_VALID(u, mem, HITf)) {
        hit = soc_mem_field32_get(u, mem, e, HITf);
        if (hit) {
 ...
 */
STATIC void
_bcm_td2_alpm_ent_data_parse(int unit, _bcm_defip_cfg_t *lpm_cfg, int *nh_ecmp_idx,
        ...
        for (tmp_idx = 0; tmp_idx < step_count; tmp_idx++) {
            ...
            if (ipv6 && tmp_idx) {
                ...
                if (soc_feature(unit, soc_feature_ipmc_defip)) {
                    ...
                    ipmc_route = soc_mem_field32_get(unit, L3_DEFIPm, lpm_entry,
                                                        MULTICAST_ROUTE0f);
                    if ((ipmc_route) &&
                            (soc_mem_field32_get(unit, L3_DEFIPm, lpm_entry,
                                                       L3MC_INDEX0f) == 0)) {
                        continue;
                    }
                }
    ...
    for (idx = 0; idx < idx_end; idx++) {
        ...
        for (tmp_idx = 0; tmp_idx < step_count; tmp_idx++) {
            ...
            if (tmp_idx) {
                ...
                if (soc_feature(unit, soc_feature_ipmc_defip)) {
                    /* Skip reserved ipmc entry */
                    ipmc_route = soc_mem_field32_get(unit, L3_DEFIP_PAIR_128m, lpm_entry,
                                                     MULTICAST_ROUTEf);
                    if ((ipmc_route) &&
                            (soc_mem_field32_get(unit, L3_DEFIP_PAIR_128m, 
                                                 lpm_entry, L3MC_INDEXf) == 0)) {
                        continue;
                    }
                }
#endif
        ...
        bcm_if_t *intf_array)
{
    ...
    if ((l3_bk_info->riot_enable) || BCMI_L3_ECMP_IS_MULTI_LEVEL(unit)) {
        if (ecmp->ecmp_group_flags & BCM_L3_ECMP_OVERLAY) {
            block_base_ptr = 0;
            max_block_base_ptr = (l3_bk_info->l3_ecmp_rh_overlay_entries >> 6)
                                  - num_blocks;
            LOG_DEBUG(BSL_LS_SOC_L3, (BSL_META_U(unit,
                "Total_blocks = %d , num_blocks = %d\n "),
                total_blocks, num_blocks));
            ...
            block_base_ptr = (l3_bk_info->l3_ecmp_rh_overlay_entries >> 6);
        ...
        bcm_if_t *old_intf_array)
{
    ...
    rv = _bcm_td2_ecmp_rh_dynamic_size_decode(flow_set_size, &num_entries);
    if (BCM_FAILURE(rv)) {
        goto cleanup;
    }
        ...
        int new_ecmp_group)
{
    ...
    old_ecmp.ecmp_intf = old_ecmp_group + BCM_XGS3_MPATH_EGRESS_IDX_MIN;
    ...
    new_ecmp.ecmp_intf = new_ecmp_group + BCM_XGS3_MPATH_EGRESS_IDX_MIN;
    ...
    if ((l3_bk_info->riot_enable || BCMI_L3_ECMP_IS_MULTI_LEVEL(unit)) &&
       (l3_bk_info->l3_ecmp_rh_overlay_entries > 0)) {
        ...
        LOG_CLI((BSL_META_U(unit,
              ...
              l3_bk_info->l3_ecmp_rh_overlay_entries/64));
