                       ...
                       bcm_l3_host_traverse_cb notify_cb, void *user_data)
{
    ...
    for (tbl = 0; tbl < num_tables; tbl++) {
        ...
        for (idx = 0; idx <= idx_max; idx++) {
            ...
            BCM_IF_ERROR_RETURN(_bcm_tr3_l3_ism_del(unit, &l3cfg));
...
_bcm_tr3_nh_update_match(int unit, _bcm_l3_trvrs_data_t *trv_data)
{
    ...
    for (chunk_index = 0; chunk_index < num_chunks; chunk_index++) {
        ...
        for (idx = idx_min; idx <= idx_max; idx++) {
            ...
            if (BCM_XGS3_L3_BLACK_HOLE_NH_IDX == idx) {
                continue;
            }
            ...
            _bcm_xgs3_nh_entry_parse(unit, ing_entry_ptr, egr_entry_ptr, idx, 
                                     &nh_entry, NULL);
        ...
        bcm_if_t *intf_array, int *member_id_array)
{
    ...
    for (i = 0; i < intf_count; i++) {
        if (BCM_XGS3_L3_EGRESS_IDX_VALID(unit, intf_array[i])) {
            nh_index = intf_array[i] - BCM_XGS3_EGRESS_IDX_MIN;
        } else if (BCM_XGS3_DVP_EGRESS_IDX_VALID(unit, intf_array[i])) {
            nh_index = intf_array[i] - BCM_XGS3_DVP_EGRESS_IDX_MIN;
        } else {
            return BCM_E_PARAM;
        }
        ...
        bcm_l3_egress_t *l3_egress) 
{
    ...
    */
    if (soc_feature(unit, soc_feature_ing_l3_next_hop_encoded_dest)) {
        BCM_IF_ERROR_RETURN(
            bcmi_l3_nh_dest_set(unit, l3_egress, &nh_dest));
            soc_mem_field32_set(unit, INITIAL_ING_L3_NEXT_HOPm, &ing_nh, DESTINATIONf,
                nh_dest);
    } else 
    ...
    if (soc_feature(unit, soc_feature_virtual_port_routing)) {
        ...
        if (l3_egress->encap_id > 0 &&
            ...
            l2_egr_nh_index = l3_egress->encap_id - BCM_XGS3_EGRESS_IDX_MIN;
            SOC_IF_ERROR_RETURN(soc_mem_read(unit, EGR_L3_NEXT_HOPm,
                        MEM_BLOCK_ANY, l2_egr_nh_index, &l2_egr_nh));
            if (2 != soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm, &l2_egr_nh,
                        ENTRY_TYPEf)) {
                return BCM_E_PARAM;
            }
...
{
    bcm_module_t module, module_out;
...
#endif
    {
        ...
        if (soc_mem_field32_get(unit, ING_L3_NEXT_HOPm, ing_nh, Tf)) {
            ...
            l3_egress->trunk = soc_mem_field32_get(unit, ING_L3_NEXT_HOPm, ing_nh,
                    TGIDf);
        } else {
