...
#if (defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined (BCM_SAND_SUPPORT))
/* { */
...
int         rx_dv_count;     /* Debug: count number of DVs used */
...
int _bcm_rx_shutdown(int unit)
{
    ...
    if (i == BCM_CONTROL_MAX) {
        BCM_IF_ERROR_RETURN(bcm_rx_pool_cleanup());
    }
...
sal_usecs_t last_fill_check = 0;    /* Last time tokens checked */
...
/* } */
    ...
    RX_INFO((BSL_META_U(unit,"%s,h1 0x%x, h2 0x%x, h3 0x%x, h4 0x%x, not filled reason 0x%x"
                ...
                "oamCouterSz %u, oam_counter0.lwr %u, oam_counter0.upr %u, counter_object_id0 %u"
                        "\n"),
             __FUNCTION__,oh.u1.h1,oh.u2.h2,oh.u3.h3,oh.u4.h4,pkt->rx_reason,
...
/* } */
#endif /* (defined(BCM_ESW_SUPPORT) || defined(BCM_SBX_CMIC_SUPPORT) || defined (BCM_SAND_SUPPORT)) */
...
/* C model's version of the process_packet function */
void rx_cmodel_process_packet(int unit, bcm_pkt_t *pkt)
{
    volatile rx_callout_t *rco;
    bcm_rx_t              handler_rc;
    int                   handled = FALSE;
...
/* { */
    ...
    if (SOC_UNIT_VALID(unit)) {
         dnx_rx_packet_parse(unit, pkt, 1);
	}
/* } */
#endif
/* } */
